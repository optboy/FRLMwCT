...
class BD:
    ...
    def set_master(self):
        ...
        cpx.variables.add(names=["z"], ub=[10e8])
    ...
    def solve_master_iterative(self, time_limit=3600):
        ...
        len_path = [sum([len(self.paths[k]) for k in self.prob.K.keys()])]
        ...
        while True:
            if time.time() - self.start_time > self.time_limit:
                ...
                self.master.solve()
                print(self.master.solution.get_objective_value())
                ...
                alpha_k, beta_k, cut_gen_time, labeling_time, lp_solve_time = cut_generation(self.prob, self.master.solution.get_values(), 
                            self.sub_models, self.paths, self.prob.shortest_time, self.prob.use_hrstc, self.start_time, self.time_limit, self.thread_executor)
                num_path = sum([len(self.paths[k]) for k in self.prob.K.keys()])
                len_path.append(num_path)
                ...
                pi_l = get_pi(self.prob, corrected_alpha_k)

                cut = get_cut(self.prob, pi_l)
                self.generated_cuts.append((cut, sum(beta_k), self.master.solution.get_values()))
...
# cut_generation v3
def cut_generation(prob, x, sub_models, paths, shortest_time, use_hrstc, start_time, time_limit, thread_executor=5):
    cut_gen_start_time = time.time()
    done = False
    ...
    while not done:
        ...
        EN_list = [[O, D, N, R, ADJ_MAT, np.array(ALPHA), CT, QT, CS, g_star, Q, UH, False]
                   for (O, D, N, R, ADJ_MAT, DECAY, CT, QT, CS, Q), ALPHA, UH in
                   zip(fwd_EN_INFO, fwd_alpha_k, fwd_use_hustc)]
        ...
        EN_list = [[O, D, N, R, ADJ_MAT, np.array(ALPHA), CT, QT, CS, g_star, Q, UH, False]
                   for (O, D, N, R, ADJ_MAT, DECAY, CT, QT, CS, Q), ALPHA, UH in
                   zip(bwd_EN_INFO, bwd_alpha_k, bwd_use_hustc)]
...
def labeling_numba(O, D, N, R, ADJ_MAT, ALPHA, CT, QT, CS, W, CTR, use_hrstc, fst_q):
    # Label definition:
    ...
    # return A, T, B, DIST, PREV, VISIT, TREAT, VALID
    labels = [(1,2)]
    paths = [[1,2,3]]
    labels.pop()
    paths.pop()
...
class LazyCutGenCallback(cplex.callbacks.LazyConstraintCallback):
    def __call__(self):
        if time.time() - self.start_time > self.time_limit:
            ...
            cut = get_cut(self.prob, pi_l)
            self.generated_cuts.append((cut,sum(beta_k),self.get_values()))
            
            self.add(cut, 'L', sum(beta_k))
            self.num_cuts += 1
